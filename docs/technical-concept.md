...
Spieleabend ist eine einzelne Next.js-16.3.1-Anwendung mit App Router, React 19.2.4 und strengem TypeScript. Implementiert sind die Registrierung und die atomare Vergabe des Startguthabens. Konten und Starttransaktionen liegen im Speicher eines Node-Prozesses. Anmeldung, Sitzungen, Spiele, Leaderboard und REST Route Handlers sind geplant.
...
```text
Server Components ──────────────────────────────┐
Client Component ── Server Action ── Service ── RuntimeUnitOfWork
                                                │
...
| `src/domain/` | Frameworkunabhängige Konto-, Registrierungs- und Creditregeln. |
| `src/server/services/` | Validierung und Ablauf der Registrierung sowie die atomare Unit of Work. |
| `src/server/repositories/` | Schmale Lesezugriffe auf Konten und Credittransaktionen. |
| `src/server/store/` | Private Maps, E-Mail-Index und atomarer Zustandswechsel. |
| `src/server/auth/` | Aktuell das Argon2id-Passwort-Hashing; Sitzungen sind noch nicht implementiert. |
...
IDs werden serverseitig als undurchsichtige UUIDs erzeugt; Zeitpunkte werden als UTC-ISO-Zeichenketten gespeichert. Das Ergebnis des Registrierungsservices enthält nur Anzeigename und Credits. E-Mail-Adresse, Passwort-Hash und interne IDs werden nicht an die Darstellung weitergegeben.

Aktuell gilt die Beziehung `Account 1 ── 1 CreditTransaction`, weil nur die einmalige Starttransaktion implementiert ist. Mit den späteren Funktionen ist folgendes Zielmodell vorgesehen:
...
Session, Spielrunde und Transaktion gehören jeweils genau einem Konto; die Konto-ID wird bei geschützten Vorgängen aus der geprüften Sitzung abgeleitet. Nur vollständig abgerechnete Spielrunden werden gespeichert. Sitzungen und Spielrunden sind noch nicht implementiert und werden erst mit ihren jeweiligen Funktionen konkretisiert.

## Laufzeitpersistenz und Atomarität

Der exportierte `InMemoryStore` bleibt bei gewöhnlichen Importen innerhalb desselben Node-Prozesses erhalten. Seine Maps und der Index für normalisierte E-Mail-Adressen sind privat; Lesezugriffe geben Kopien zurück. Bei einem Serverneustart geht der Zustand verloren, und mehrere Prozesse würden getrennte Zustände führen.
...
Die vorgesehene HTTP/JSON-API ergänzt die Server Actions für die Bewertung der API-Kriterien. Sie ist noch nicht implementiert. Sie folgt REST-Konventionen für Ressourcen, Methoden und Statuscodes, ist wegen der geplanten serverseitigen Sitzungen jedoch nicht streng zustandslos.
...
`/` und `/register` sind statisch erzeugbare Server-Component-Seiten. Überschriften, Beschreibung, Labels und die anfängliche Formularstruktur werden als HTML vom Server geliefert. `RegisterForm` ist eine Client Component, weil sie kontrollierte Eingaben, Pending-Zustand und Rückmeldungen verwaltet. Ihre erste Darstellung wird serverseitig vorgerendert und anschließend im Browser hydratisiert; die Anwendung ist daher keine reine CSR-Anwendung.

`Button`, `Input` und `Card` besitzen keine eigene `"use client"`-Grenze. Sie bleiben serverkompatibel und werden nur innerhalb eines Client-Teilbaums Teil des Browserbundles. Zukünftige geschützte Seiten werden für sitzungsabhängige Daten dynamisch auf dem Server gerendert. Server Components greifen dabei direkt auf serverseitige Services zu und rufen nicht die eigene REST-API auf.
...
Implementiert sind strikte Eingabevalidierung, serverseitige Kontrolle über IDs und Credits, Argon2id-Hashing, private Store-Strukturen, reduzierte Ergebnisobjekte, sichere Fehlermeldungen und atomare Schreibvorgänge. Die Eindeutigkeit der normalisierten E-Mail-Adresse wird an der atomaren Schreibgrenze erneut geprüft.

Anmeldung, Authentifizierung, Autorisierung, Eigentumsprüfungen, Session-Cookies und Logout sind noch nicht implementiert. Vorgesehen sind undurchsichtige, zufällige serverseitige Sitzungen mit Ablaufzeit sowie HttpOnly-, SameSite- und in Produktion Secure-Cookies. Unsichere cookie-authentifizierte HTTP-Methoden benötigen zusätzlich eine geprüfte Same-Origin-/CSRF-Strategie. Seiten- oder Proxy-Weiterleitungen verbessern nur die Navigation; Actions, Route Handlers und Services prüfen Identität und Berechtigung erneut. Passwort- und Sitzungsdaten sowie interne Fehlerdetails dürfen nicht in Client Props, öffentlichen Antworten oder Logs erscheinen.
